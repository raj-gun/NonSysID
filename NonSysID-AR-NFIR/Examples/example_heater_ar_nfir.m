...
u = rand(n, 1); % Random input signal, u(t) ~ U(0,1), the range of the normalised electric power
y = heater_ar_nfir_model(n, u, e); % Simulate the Hammerstein model
%% System Identification - NonSysID
tt_splt = 1:100; %Training samples of data
u_ID=u(tt_splt);
y_ID=y(tt_splt);
...
% Maximum order of polynomial nonlinearity considered for the NAR and NFIR parts respectively
nl_ord_AR=1; nl_ord_NFIR=2; 
...
[model, Mod_Val_dat, iOFR_table_lin, iOFR_table_nl, best_mod_ind_lin, best_mod_ind_nl, val_stats] = ...
    NonSysID_AR_NFIR(mod_type,u_ID,y_ID,na1,na2,nb1,nb2,nl_ord_AR,nl_ord_NFIR,is_bias,n_inpts,KSA_h,RCT,x_iOFR,stp_cri,D1_thresh,displ,sim,parall);
toc

% Print ARX/AR-NFIR NARX models
