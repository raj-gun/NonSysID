function [theta,ms_press_e,ERR,ERR_itr,MS_PRESS_E,MS_PRESS_E_itr,alpha,trm_chsn_logic_ind,trm_chsn_ind,ind_orth_wrt,ind_orth_wrt_org] ...
    = OLS_orthogonalisation_reg_press_frc(X,Y,lambda_ini,frc_ind,reg_itr_end,lamda_stp,stp_cri,alph)
%#codegen
...
for m = 2:n_terms  %(frc_ind(end)+1)
    ...
    alpha_temp = zeros(i_n , r_n); % alpha_rm^{i}
    ERR_temp = zeros(i_n , 1); % [ERRm]^{i}
    Wm_temp = zeros(dat_len , i_n); % Wm^{i}
    g_temp = zeros(i_n , 1); % gm^{i}
    
    error_pred_temp = zeros(dat_len , i_n); % Em^{i} , Error predicted of each remaining regressor
    PRESS_W_temp = zeros(dat_len , i_n); % PRESS_Wm^{i} , PRESS error weighting of each remaining regressor
    MS_PRESS_E_temp = zeros(i_n , 1); % Jm^{i} , Mean Square PRESS Error of each remaining regressor
    
    Pi = X(:,trm_lft_chsn); %regressors to be orthogonalised
    
    for i = 1:i_n
        
        %for r = 1:r_n  % Evaluate alpha_rm^{i}
        %    alpha_temp(i,r) = ( Pi(:,i)' * Wm(:,r) ) / ( Wm(:,r)' * Wm(:,r) );
        %end
		
		% -------------- Evaluate alpha_rm^{i} ----------------
		alpha_temp(i, :) = ( Pi(:,i)' * Wm(:, 1:r_n) ) ./ ( diag( Wm(:, 1:r_n)' * Wm(:, 1:r_n) ) )';
		% -----------------------------------------------------
        
        % -------------- Evaluate Wm^{i} ----------------
        alpha_Wm_i = alpha_temp(i,:) .* Wm(:,1:r_n);
        alpha_sum_i = sum(alpha_Wm_i,2);
        Wm_temp(:,i) = Pi(:,i) - alpha_sum_i;
        % -----------------------------------------------
        
        % -------------- Evaluate gm^{i} ----------------
        g_temp(i) = ( Wm_temp(:,i)' * Y ) / ( Wm_temp(:,i)' * Wm_temp(:,i) + lambda(m) );
        % -----------------------------------------------
        
        % -------------- Evaluate [ERRm]^{i} ----------------
        ERR_temp(i) = (g_temp(i)^2) * ( Wm_temp(:,i)' * Wm_temp(:,i) + lambda(m) ) / sigma;
        % ---------------------------------------------------
        
        % --------------- Evaluate Em^{i} -----------------
        error_pred_temp(:,i) = error_pred - Wm_temp(:,i)*g_temp(i);
        % --------------------------------------------------
        
        % --------------- Evaluate PRESS_Wm^{i} -----------------
        PRESS_W_temp(:,i) = PRESS_W -  (Wm_temp(:,i).^2) ./ ( Wm_temp(:,i)' * Wm_temp(:,i) + lambda(m) );
        % -------------------------------------------------------
    ...
    if m > 2 % stopping criteria invoked only after two terms are selected
        switch stp_cri
            case 'PRESS'
                if (MS_PRESS_E(m) >= MS_PRESS_E(m-1)) % PRESS statistic
                    n_end = 1;
                    break;
                end
                
            case 'APRESS'
                if (APRESS(m) >= APRESS(m-1)) % APRESS
                    n_end = 1;
                    break;
                end
                
            case 'BIC'
                if (BIC(m) >= BIC(m-1)) % BIC
                    n_end = 1;
                    break;
                end
                
            otherwise
                if (sum(ERR) >= stp_cri) % SERR
                    n_end = 0;
                    break;
                end
        end
