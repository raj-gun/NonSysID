function [theta,X_main,trm_chsn_lin,trm_chsn_lin_org,n_lin_trms_org,trm_lag_char_lin_incld,trm_chsn_nl,trm_lag_char_tot,...
    iFRO_table_lin,iFRO_table_nl,ERR_table_lin,ERR_table_nl,best_mod_ind_lin,best_mod_ind_nl,Mod_Val_dat]...
    = Sys_ID_SEM_PRESS_LReg_iFRO(RMO,err_type,mod_type,U_delay_mat,k,X,Y,Y_sim,Sim_ini_val,min_dyn_ord_u,max_dyn_ord_u,min_dyn_ord_y,max_dyn_ord_y,nl_ord_max,is_bias,inpt0,reg_itr_end,lambda_ini,lamda_stp,stp_cri,alph,D1_thresh,displ,x_iFRO)
%#codegen
%%
warning('off');
size_X = size(X);
...
[trm_lag_char_lin, X] = iFRO_trm_char(no_outpts, max_dyn_ord_y, no_inpts, inpt0, max_dyn_ord_u, n_terms_y, n_terms_u, mod_type, is_bias, min_dyn_ord_y, min_dyn_ord_u, X, dat_len);
size_X = size(X);
...
[iFRO_table_lin, trm_chsn_lin, best_press, best_msse, theta_lin, ERR_table_lin, best_mod_ind_lin] = iFRO_lin_SysID...
    (n_lin_trms_rem, is_bias, trm_chsn_lin, n_terms, k, X_main, Y, Y_sim, n_terms_y, U_delay_mat, n_lin_trms_org,...
    y_lag_lin_srt, trm_lag_char_lin, reg_itr_end, lambda_ini, lamda_stp, stp_cri, alph, displ, dat_len, x_iFRO, best_msse_prev, best_press_prev);
...
if nl_ord_max >= 2
    %disp('Composing nonlinear model terms');
    
    % ------------- Select RMO (Reduced Model Order) method ---------------
    [trm_lag_char_tot, X_main_org, trm_chsn_lin_org, trm_chsn_lin, n_lin_trms_org, n_lin_trms, trm_chsn_nl, n_nl_trms, trms]...
        = RMO_sel(RMO, trm_lag_char_lin_incld, X_main, trm_chsn_lin_org, trm_chsn_lin, n_lin_trms_org, n_lin_trms, nl_ord_max, trm_lag_char_lin, X, Y, alph, is_bias, dat_len);
    % ---------------------------------------------------------------------
    ...
    %--------
    [trm_chsn_lin_temp, iFRO_table_nl, trm_chsn_nl, best_mod_ind_nl, ERR_table_nl, trm_lag_char_tot_nl, theta_nl, X_main_nl, Mod_Val_dat_nl] = iFRO_nl_SysID...
        (trm_chsn_lin, n_trms_rem, is_bias, trms, bias_trms_ind, k, X, X_main_org, Y, Y_sim, n_terms_y, U_delay_mat, trm_chsn_lin_org,...
        n_lin_trms_org, n_lin_trms, y_lag_lin_srt, nl_ord_max, trm_chsn_nl, trm_lag_char_tot, reg_itr_end, lambda_ini, lamda_stp, stp_cri, alph, D1_thresh, displ, dat_len, x_iFRO, best_msse_prev, best_press_prev);
    %-------
 ...
 % ---------------------------------------------------------------------
    ...
    iFRO_table_nl = 0; ERR_table_nl = 0; trm_lag_char_tot = trm_lag_char_lin_incld;
