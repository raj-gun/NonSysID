function [iOFR_table_lin, trm_chsn_lin, best_press, best_msse, theta_lin, ERR_table_lin, best_mod_ind_lin] = iOFRs_lin_SysID_P...
...
while true
    ...
    mod_eliminate = zeros(n_lin_trms_rem,1); %Models to eliminate
    for k=1:n_lin_trms_rem %eleminate models that only contain a bias term
        if size( iOFR_table_lin{k,9} , 1) == 1
            if strcmp(iOFR_table_lin{k,9}.Properties.RowNames{1} , 'bias')
                 mod_eliminate(k,1) = k;               
            end
        end
    end
