...
Steps 2 and 3 in $\text{iOFR}_{S}$ incorparates simulation-based model selection. In Step 2, each candidate model $\Tilde{m}_i \in \Tilde{\mathcal{M}}$, $i=1,\dots,p$ is tested for stability using two constant inputs: $u^{[0]}(t) = 0 \ \forall t$ and $u^{[1]}(t) = 1 \ \forall t$. The corresponding simulated outputs $\hat{y}^{[0]}(t)$ and $\hat{y}^{[1]}(t)$ must remain bounded with a small variance, i.e., $\mathbb{E}[\hat{y}^{[j]}(t)] \in \mathbb{R}$ and $\text{Var}(\hat{y}^{[j]}(t)) \leq \varepsilon$, typically with $\varepsilon = 10^{-2}$. For $u^{[0]}(t)$, the mean equals the model’s bias term $\beta$. In Step 3, the Bayesian Information Criterion (BIC) [@Schwarz1978; @Stoica2004] selects the best model $\overline{m} \in \mathcal{M}$ based on the mean squared simulated error (MSSE) [@Piroddi2003]. In the latter of this paper, $\text{iOFR}_{S}$ is represented in functional form as $( \mathcal{M}, \overline{m} ) = \text{iOFR}_{S}( \mathcal{D}, \mathcal{P}, \mathbf{Y})$.

Let $\mathcal{D'} = Y \cup U$ be the set of candidate linear terms from past outputs $Y$ and inputs $U$ for ARX model identification. For NARX models, the candidate set $\mathcal{D}''$ includes both linear and nonlinear terms, so $\mathcal{D}'' \supset \mathcal{D'}$. As the number of terms in $\mathcal{D'}$ and nonlinearity increases, $|\mathcal{D}''|$ can grow exponentially [@billings2013a], increasing computational cost for iOFR/$\text{iOFR}_S$. Appropriately reducing this set to $\mathcal{D}''_R \subset \mathcal{D}''$ can reduce complexity. Likewise, minimizing redundant terms in the initial pre-select set $\mathcal{P}$ can speed up convergence toward an optimum model [@guo2015a]. This section discusses how to effectively reduce $\mathcal{D}''$ and optimize $\mathcal{P}$ for the first iteration to improve $\text{iOFR}_S$ efficiency.
