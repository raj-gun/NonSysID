...
- **RCT Method 2**: This method, as proposed in [@guo2015a], identifies an appropriate $\mathcal{P}$ for the first iteration of the iOFR algorithm by identifying an overfitted NARX model $\overline{m_{0}''}$ using OFR. Terms of $\overline{m_{0}''}$ are used to form $\mathcal{P}$ as it is likely to contain some correct terms and results in fewer redundant terms than choosing $\mathcal{P} \subseteq \mathcal{D}''$ directly.
