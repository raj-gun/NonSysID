...
In the original iOFR algorithm [@guo2015a], model selection was based on the one-step-ahead prediction. The implementation of iOFR in the `NonSysId' extends this by incorporating simulation-based model selection ($\text{iOFR}_{S}$) to ensure simulation stability and improve long-horizon prediction accuracy. There are two simple tests done to ensure simulation stability (bounded-input bounded-output). Using a pre-select term list $\mathcal{P} = \{ \phi_1 , \dots, \phi_p  \}$, the iFRO algorithm explores $p$ orthogonalization paths. Each path produces a candidate model $\Tilde{m}_i \in \Tilde{\mathcal{M}}$, $i=1,\dots,p$ which is tested for stability using two constant inputs: $u^{[0]}(t) = 0 \ \forall t$ and $u^{[1]}(t) = 1 \ \forall t$. The corresponding simulated outputs $\hat{y}^{[0]}(t)$ and $\hat{y}^{[1]}(t)$ must remain bounded with a small variance, i.e., $\mathbb{E}[\hat{y}^{[j]}(t)] \in \mathbb{R}$ and $\text{Var}(\hat{y}^{[j]}(t)) \leq \varepsilon$ ($j = 0 \text{or} 1$), typically with $\varepsilon = 10^{-2}$. For $u^{[0]}(t)$, the mean equals the model’s bias term $\beta$. Afterwards, from a set of stable models $\mathcal{M}$, the Bayesian Information Criterion (BIC) [@Schwarz1978; @Stoica2004] selects the best model $\overline{m} \in \mathcal{M}$ based on the mean squared simulated error (MSSE) [@Piroddi2003]. $\mathcal{P}$ is updated with the term of $\overline{m}$, and the next iteration begins. In the latter part of this paper, $\text{iOFR}_{S}$ is represented in functional form as $( \mathcal{M}, \overline{m} ) = \text{iOFR}_{S}( \mathcal{D}, \mathcal{P}, \mathbf{Y})$.
