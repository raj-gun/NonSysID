function [iOFR_table_lin,trm_chsn_lin,best_press,best_msse,...
    theta_lin,ERR_table_lin,best_mod_ind_lin] = iOFRs_lin_SysID_P_i(...
...
%IOFRS_LIN_SYSID_P_I Parallel linear iOFR model selection for ARX-i.
%
% Each forced-term candidate model is evaluated independently using PARFOR.
% Candidate ranking, BIC calculation and iterative model reduction remain
% serial so the model-selection logic matches iOFRs_lin_SysID_i.
...
while true
    ...
    parfor j = 1:n_lin_trms_rem
        if is_bias == 0
            ...
            % If a bias is required, choose it first, followed by the
            % currently forced input-only regressor.
            frc_ind_temp = [(n_terms+1),trm_chsn_lin(j)];
        end

        iOFR_table_lin_temp = OFR_lin_i(...
            X_main,Y,Y_sim,frc_ind_temp,U_delay_mat,...
            trm_lag_char_lin,stp_cri,...
            D1_thresh,alph,displ);

        iOFR_table_lin(j,:) = iOFR_table_lin_temp;
    end

    % Eliminate models that contain only a bias term.
